    ...
    <div className="flex-1 overflow-auto">
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-4 gap-4">
          ...
          {/* Întâlniri azi → Events */}
          <Link href="/events" className="block group">
            <StatCard
              icon={<Calendar size={18} />}
              label="Întâlniri azi"
              value={todayEvents?.length || 0}
              sub={(todayEvents as any)?.[0] ? format(new Date((todayEvents as any)[0].starts_at), 'HH:mm') : 'Nimic azi'}
              color="blue"
              clickable
            />
          </Link>

          {/* Sarcini active → Tasks, cu badge expirate separat */}
          <div className="relative">
            <Link href="/tasks" className="block group">
              <StatCard
                ...
                sub={overdueCount > 0 ? `${overdueCount} expirate` : 'La zi ✓'}
                color={overdueCount > 0 ? 'red' : 'green'}
                clickable
              />
            </Link>
            {/* Badge expirate clickabil separat */}
            {overdueCount > 0 && (
              <Link
                href="/tasks?filter=overdue"
                className="absolute top-3 right-3 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full hover:bg-red-600 transition-colors z-10"
                title="Vezi sarcinile expirate">
                {overdueCount} expirate
              </Link>
            )}
          </div>
          ...
          <div className="card overflow-hidden">
            ...
            <div>
              {(pendingTasks || []).slice(0, 5).map((t: any) => {
                ...
                return (
                  <Link key={t.id} href="/tasks" className="px-4 py-3 border-b border-gray-100 last:border-0 flex items-center gap-3 hover:bg-gray-50 transition-colors">
          ...
          <div className="card overflow-hidden">
            ...
            <div>
              {(recentLeads || []).map((l: any) => {
                ...
                return (
                  <Link key={l.id} href="/pipeline" className="px-4 py-3 border-b border-gray-100 last:border-0 flex items-center gap-3 hover:bg-gray-50 transition-colors">
            ...
            <div className="flex flex-wrap gap-3 p-4">
              {(todayEvents || []).map((e: any) => (
                <Link key={e.id} href="/events"
                  className="flex items-center gap-2 bg-[#e8f0ee] rounded-lg px-3 py-2 hover:bg-[#d4e8e2] transition-colors">
...
function StatCard({
  icon, label, value, sub, color, clickable
}: {
  ...
  return (
    <div className={`card relative overflow-hidden transition-all ${clickable ? 'hover:shadow-md hover:-translate-y-0.5 cursor-pointer' : ''}`}>
